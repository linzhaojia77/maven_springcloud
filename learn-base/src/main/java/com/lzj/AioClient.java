...
public class AioClient {
    CountDownLatch running = new CountDownLatch(20);
    public static void main(String[] args) throws Exception {
        AsynchronousSocketChannel socketChannel = AsynchronousSocketChannel.open();
        socketChannel.connect(new InetSocketAddress("127.0.0.1",8089), socketChannel,new ConnectFinishHandler());
        CountDownLatch running = new CountDownLatch(1);
        running.await();
        System.out.println("1次数到了，不发了");
        socketChannel.close();
    }
    ...
    static class ConnectFinishHandler implements CompletionHandler<Void, AsynchronousSocketChannel> {
        ...
        public void completed(Void result, AsynchronousSocketChannel socket) {
            ...
            String readStr = "Input something:";
